    ...
    public class AuthenticationFilterWrapperTests
    {
        ...
        public async Task WrapperResolvesAuthenticationFilterFromDependencyScope()
        {
            ...
            builder.Register(c => new TestAuthenticationFilter(c.Resolve<ILogger>()))
                .AsWebApiAuthenticationFilterWhen((hcd, had) => typeof(TestController).IsAssignableFrom(had.ControllerDescriptor.ControllerType) && had.ActionName == nameof(TestController.Get), FilterScope.Action, order: 0)
                .InstancePerRequest()
